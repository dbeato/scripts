...
$Table | Sort-Object 'Size (Bytes)' -Descending | Export-CSV -Path $TempLocation\$ExportedFilesCSV -NoTypeInformation -UseCulture

# Export the largest 100 results into html format
$Table |
    Sort-Object 'Size (Bytes)' -Descending |
...
# Export the table to a new CSV
$Table | Sort-Object 'Size (Bytes)' -Descending | Export-CSV -Path $TempLocation\$ExportedFoldersCSV -NoTypeInformation -UseCulture
...
$WMIDiskInfo = Get-CimInstance -ClassName Win32_Volume -Property Capacity,FreeSpace,DriveLetter | Where-Object {$_.DriveLetter -eq $env:SystemDrive} | Selesact-Object Capacity,FreeSpace,DriveLetter
